...
10) After performing step 9), change the value of the [Client]/IsolationMode tag at least once so the tag change script will set the correct value for the database and tag provider.

11) Save the project.

12) ILS SFC charts need to be started in a special way, and charts involving window UIs will not work from the Designer. If you don't have a launcher window already defined, in the Designer mark the SFC/ExampleLauncher window as "Open on Startup" and use the button action scripts as models for your own application.